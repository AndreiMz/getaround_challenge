...
def model_credit_debit(rentals)
    result_with_credit_debit = {
        rentals: []
    }

    rentals[:rentals].each do |rental|
        total = rental[:price]
        commission = total * 3/10

        result_with_credit_debit[:rentals] << {
            id: rental[:id],
            actions: [
                {
                    who: 'driver',
    ...
    make_output(result_with_credit_debit)

    result_with_credit_debit
end

rentals = compute_rentals_price
rentals = split_commision(rentals)
model_credit_debit(rentals)